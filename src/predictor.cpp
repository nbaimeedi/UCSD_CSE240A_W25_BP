...
void train_tournament(uint32_t pc, uint8_t outcome)
{
  ...
  tournament_g_prediction = (tournament_g_bht[index] == ST)? 1 : 0;

  index = tournament_l_lht[pc_lower_bits];
  tournament_l_prediction = (tournament_l_bht[index] == WN)? 0 : 1;
  tournament_l_prediction = (tournament_l_bht[index] == SN)? 0 : 1;
  tournament_l_prediction = (tournament_l_bht[index] == WT)? 1 : 0;
  tournament_l_prediction = (tournament_l_bht[index] == ST)? 1 : 0;
